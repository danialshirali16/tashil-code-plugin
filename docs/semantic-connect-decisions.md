...
`SEMANTIC_RECIPE_SCHEMA_VERSION` (currently 1).
...
- Existing schema-v4 connections keep generating byte-identical output; there
  is no migration to run and no downgrade cliff to document.
...
| --- | --- | --- |
| `maxTargetPathDepth` | 2 | One level of nesting (`confirmAction.label`) |
| `maxLocatorDepth` | 8 | Bounded descendant search for authoring |
| `maxNestedSources` | 64 | Keeps the value list and metadata bounded |
| `maxBindings` | 64 | Bounds persisted recipe size |
| `maxSerializedLength` | 32 000 chars | Hard cap on persisted metadata |

Deeper object nesting stays **visible as `unsupported`**, never silently
dropped.
...
- **Arrays are out of scope for v1.** Array-of-object props (e.g.
  `actionButtons: Item[]`) remain visibly `unsupported`.
