...
| **Design Health** (`src/design-health/`) | Canvas/frame-level auditing, token linting, auto-binding, and library update/deprecation inspection and updates in Figma Design mode. | `src/design-health/token-audit.ts`, `src/main/design-health-adapter.ts`, `src/views/DesignHealthView.tsx` | [section-design-health.md](section-design-health.md) | Pure core in `src/design-health/` must never import `@figma/plugin-typings`. Successful token-binding and library-update batches must checkpoint once with `figma.commitUndo()`. |
...
```text
   ...
   │ Layout/Style buckets │   │ → one .tsx module    │   │ authoring·reconcile  │   │ reconciliation            │   │ deprecation detection      │
   └─────────────────────┘   └──────────┬──────────┘   └──────────┬──────────┘   └─────────────┬─────────────┘   └──────────────┬──────────────┘
                                        │                         │                            │                                │
                                        └────────────┬────────────┘                            │                                │
                                                     ▼                                         │                                │
...
- **Design Health** audits selected canvas frames/layers, identifying unbound raw colors/spacings/radii with token recommendations, detecting and applying remote component updates, tracking deprecated instances, and batching mutations with single-step undo.
...
- [Figma Editor Modes — Design vs Dev Mode](section-editor-modes.md) — the
  ...
  runtime APIs.
- [Design Health — How It Works](section-design-health.md) — frame/canvas auditing, token linting, auto-binding, library updates, and deprecation tracking.
